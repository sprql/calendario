...
function sprintIdCell(ctx, date) {
    ...
    sprintId = Math.floor((sprintId/2)) - 1;
    var sprintAlphaId = alpha[sprintId].toUpperCase();

    var cr = 5;
    var dv = ctx.group();
    dv.rect(wc, wc);

    var t = dv.text(sprintAlphaId);
    t.move(wc / 2, 0);
    t.font({
        family:   fontFamily,
...
function renderMonth(ctx, year, month, highlightDays) {
    ...
    for (var d = 0; d < 7; d++) {
        ...
        var dv = dayCell(daysMatrix, dayShortNames[d], className);
        dv.move(wc * d, yk * wc);
        dv.get(1).font({ size: baseFontSize / 2, leading: '3em' });
    }
    ...
    for (var d = 0; d < (weekDay - 1); d++) {
        var dv = dayCell(daysMatrix, null, 'day of-prev-month');
        dv.move(d * wc, yk * wc);
    }

    for (var d = 1; d < 32; d++) {
        var date = new Date(month + '/' + d + '/' + year);
